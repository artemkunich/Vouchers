...
public sealed class JWTLoginNameProvider : ILoginNameProvider
{
    ...
    public string CurrentLoginName { get => _httpContextAccessor.HttpContext.User.Claims.Where(c => string.Equals(c.Type, "name", StringComparison.OrdinalIgnoreCase)).Select(c => c.Value).FirstOrDefault(); }
}