...
export const DomainOffers = ({api}) => {
    ...
    const postDomainOffer = useCallback(async (domainOffer) => {
        const domainOfferId = await api.postDomainOffer(token, domainOffer)   
        if(domainOfferId){
            await getDomainOffers(domainOffersFilter)  
        }
    }, [api, token, domainOffersFilter, domainOffers])

    const getDomainOffers = useCallback(async (query) => {
        let result = await api.getDomainOffers(token, query)  
        if(!result)
            result = []
    
        setDomainOffers(result)
    }, [api, token, domainOffers])
        ...
        <div className="container">
            ...
            <ListElement>
                ...
                <div className="col-1 d-grid mt-2 mb-2" >
                    <button className="btn" style={{backgroundColor: '#59a8fc', color: 'white'}} onClick={async () => getDomainOffers(domainOffersFilter)}>Search</button>
                </div>           
            </ListElement>
            <NewDomainOffer postDomainOffer={postDomainOffer}/>         
            {              
