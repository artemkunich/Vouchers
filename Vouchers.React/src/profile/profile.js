...
export const Profile = ({accountId, api, onCancel}) => {
    ...
    useDetail(token, accountId ?? currentDomainAccount?.id, api.getIdentityDetail, setDetail, setImgSrc, setCrop)
    ...
    const handleSubmitAsync = async () => {
        ...
        var id = await api.postIdentityDetail(token, formData) 
        var idDetail = await api.getIdentityDetail(id ?? detail.identityId)
    ...
    const handleOnAdminRoleChangeAsync = useCallback(async () => {
        const newIsAdmin = !detail.isAdmin
        const result = await api.putDomainAccount(token, {domainAccountId: accountId ?? currentDomainAccount?.id, isAdmin: newIsAdmin})
        if(result)
            setDetail({...detail, isAdmin: newIsAdmin})       
    }, [api, token, detail])

    const handleOnIssuerRoleChangeAsync = useCallback(async () => {
        const newIsIssuer = !detail.isIssuer
        const result = await api.putDomainAccount(token, {domainAccountId: accountId ?? currentDomainAccount?.id, isIssuer: newIsIssuer})
        if(result)
            setDetail({...detail, isIssuer: newIsIssuer})       
    }, [api, token, detail])

    const header = <h1 className="header mt-2 mb-3">{accountId ? detail.firstName + ' ' + detail.lastName + ' profile': identityId ? 'My profile' : 'Please register'}</h1>
        ...
        <div className="container">
            ...
            <div className="row mb-2">
                <div className="col-6">
                    ...
                    <input type="text" id="profilyFirstName" className="form-control" value={detail.firstName} onChange={handleFirstNameChange} disabled={!isEditState}></input>              
                </div>
                <div className="col-6">
                    <label htmlFor="profilyLastName" className="form-label">Last name</label>
                    <input type="text" id="profilyLastName" className="form-control" value={detail.lastName} onChange={handleLastNameChange} disabled={!isEditState}></input>                
                </div>
            </div>
            {
                currentDomainAccount && detail.accountId ? <div className="row mt-3 mb-2">
                    
                    <div className="col-1 form-check ms-3" >
                        <label htmlFor={"isIssuer"} className="form-check-label">Issuer</label>
                        <input type="checkbox" id={"isIssuer"} className="form-check-input" checked={detail.isIssuer} onChange={handleOnIssuerRoleChangeAsync} disabled={!(currentDomainAccount.isOwner || currentDomainAccount.isAdmin)}></input> 
                    </div>
                    <div className="col-1 form-check">
                        <label htmlFor={"isAdmin"} className="form-check-label">Admin</label>
                        <input type="checkbox" id={"isAdmin"} className="form-check-input" checked={detail.isAdmin} onChange={handleOnAdminRoleChangeAsync} disabled={!currentDomainAccount.isOwner}></input> 
                    </div>
                    <div className="col-1 form-check">
            ...
            <div className="row mb-2 mt-5">
                ...
                {
                    (isEditState || onCancel) && <div className="d-grid col-3">
        ...
        const effectAsync = async () => {
            if(identityId) {
                ...
                if(result.imageId)
                    ...
                    // setImgSrc(`data:image/png;base64,${result.imageId}`)
